...
namespace wan24.Crypto.Authentication
{
    ...
    public static partial class ClientAuth
    {
        ...
        public static async Task<bool> ValidateServerPublicKeySuiteAsync(PublicKeySuite serverPublicKey, CancellationToken cancellationToken)
        {
            ...
            using ISignaturePublicKey signerPublicKey = serverPublicKey.Signature.SignerPublicKey;
            if (!signerPublicKey.ID.SlowCompare(serverPublicKey.SignedPublicKey.PublicKey.ID)) throw new InvalidDataException("Server public key suite signer mismatch");
