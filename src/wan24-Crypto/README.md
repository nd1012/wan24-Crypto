...
`AddURandomSeed(Async)` only seed `/dev/urandom`. When calling 
...
Because seeding may be synchronized, there's a `RngSeeder` queue worker, which 
is a simple hosted service that seeds the given target `ISeedableRng` in 
background, using a copy of the given seeds. The `RngSeeder` may be customized 
easily by extending the type (pregnant methods are virtual).
